...
# Base environnement
        ...
        cd cred/utils/build_scripts
        docker build -f android32.Dockerfile -t cred-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t cred-gui-android .
        docker create -it --name cred-gui-android cred-gui-android bash

# Get the apk

        docker cp cred-gui-android:/opt/android/cred-gui/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/cred-gui/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
