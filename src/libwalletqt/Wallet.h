...
#include "wallet/api/wallet2_api.h" // we need to have an access to the Cred::Wallet::Status enum here;
...
public:
    ...
    enum Status {
        Status_Ok       = Cred::Wallet::Status_Ok,
        Status_Error    = Cred::Wallet::Status_Error,
        Status_Critical = Cred::Wallet::Status_Critical
    };

    Q_ENUM(Status)

    enum ConnectionStatus {
        ConnectionStatus_Connected       = Cred::Wallet::ConnectionStatus_Connected,
        ConnectionStatus_Disconnected    = Cred::Wallet::ConnectionStatus_Disconnected,
        ConnectionStatus_WrongVersion    = Cred::Wallet::ConnectionStatus_WrongVersion
    };
...
private:
    ...
    Wallet(Cred::Wallet *w, QObject * parent = 0);
    ~Wallet();
private:
    friend class WalletManager;
    friend class WalletListenerImpl;
    //! libwallet's
    Cred::Wallet * m_walletImpl;
    ...
    Cred::WalletListener *m_walletListener;
};
