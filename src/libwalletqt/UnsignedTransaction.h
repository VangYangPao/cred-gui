...
public:
    enum Status {
        Status_Ok       = Cred::UnsignedTransaction::Status_Ok,
        Status_Error    = Cred::UnsignedTransaction::Status_Error,
        Status_Critical    = Cred::UnsignedTransaction::Status_Critical
    };
    Q_ENUM(Status)
...
private:
    explicit UnsignedTransaction(Cred::UnsignedTransaction * pt, Cred::Wallet *walletImpl, QObject *parent = 0);
    ~UnsignedTransaction();
private:
    friend class Wallet;
    Cred::UnsignedTransaction * m_pimpl;
    QString m_fileName;
