...
class WalletListenerImpl : public  Cred::WalletListener
{
public:
...
{
    QFuture<Cred::Wallet::ConnectionStatus> future = QtConcurrent::run(m_walletImpl, &Cred::Wallet::connected);
    QFutureWatcher<Cred::Wallet::ConnectionStatus> *connectionWatcher = new QFutureWatcher<Cred::Wallet::ConnectionStatus>();

    connect(connectionWatcher, &QFutureWatcher<Cred::Wallet::ConnectionStatus>::finished, [=]() {
        QFuture<Cred::Wallet::ConnectionStatus> future = connectionWatcher->future();
                                              ...
                                              PendingTransaction::Priority priority)
{
    ...
    Cred::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), payment_id.toStdString(), amount, mixin_count,
                static_cast<Cred::PendingTransaction::Priority>(priority), currentSubaddressAccount(), subaddr_indices);
                                                 ...
                                                 quint32 mixin_count, PendingTransaction::Priority priority)
{
    std::set<uint32_t> subaddr_indices;
    Cred::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), payment_id.toStdString(), Cred::optional<uint64_t>(), mixin_count,
                static_cast<Cred::PendingTransaction::Priority>(priority), currentSubaddressAccount(), subaddr_indices);
...
{
    Cred::PendingTransaction * ptImpl = m_walletImpl->createSweepUnmixableTransaction();
    PendingTransaction * result = new PendingTransaction(ptImpl, this);
    return result;
}
...
UnsignedTransaction * Wallet::loadTxFile(const QString &fileName)
{
    ...
    Cred::UnsignedTransaction * ptImpl = m_walletImpl->loadUnsignedTx(fileName.toStdString());
...
{
    return QString::fromStdString(Cred::Wallet::genPaymentId());
}
...
{
    return QString::fromStdString(m_walletImpl->getDefaultDataDir()) + "/bitcred.log";
}
...
Wallet::Wallet(Cred::Wallet *w, QObject *parent)
    : QObject(parent)
    , m_walletImpl(w)
    , m_history(nullptr)
...
Wallet::~Wallet()
{
    ...
    delete m_subaddress;
    m_subaddress = NULL;
    //Cred::WalletManagerFactory::getWalletManager()->closeWallet(m_walletImpl);
