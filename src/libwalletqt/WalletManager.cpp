                                    ...
                                    const QString &language, NetworkType::Type nettype)
{
    ...
    Cred::Wallet * w = m_pimpl->createWallet(path.toStdString(), password.toStdString(),
                                                  language.toStdString(), static_cast<Cred::NetworkType>(nettype));
...
Wallet *WalletManager::openWallet(const QString &path, const QString &password, NetworkType::Type nettype)
{
    ...
    Cred::Wallet * w =  m_pimpl->openWallet(path.toStdString(), password.toStdString(), static_cast<Cred::NetworkType>(nettype));
...
Wallet *WalletManager::recoveryWallet(const QString &path, const QString &memo, NetworkType::Type nettype, quint64 restoreHeight)
{
    ...
    Cred::Wallet * w = m_pimpl->recoveryWallet(path.toStdString(), memo.toStdString(), static_cast<Cred::NetworkType>(nettype), restoreHeight);
    m_currentWallet = new Wallet(w);
    return m_currentWallet;
}
                                            ...
                                            quint64 restoreHeight)
{
    ...
    Cred::Wallet * w = m_pimpl->createWalletFromKeys(path.toStdString(), language.toStdString(), static_cast<Cred::NetworkType>(nettype), restoreHeight,
                                              ...
                                              const QString &deviceName, uint64_t restoreHeight, const QString &subaddressLookahead)
{
    ...
    Cred::Wallet * w = m_pimpl->createWalletFromDevice(path.toStdString(), password.toStdString(), static_cast<Cred::NetworkType>(nettype),
...
{
    return Cred::Wallet::maximumAllowedAmount();
}
...
{
    return QString::fromStdString(Cred::Wallet::displayAmount(amount));
}

quint64 WalletManager::amountFromString(const QString &amount) const
{
    return Cred::Wallet::amountFromString(amount.toStdString());
}

quint64 WalletManager::amountFromDouble(double amount) const
{
    return Cred::Wallet::amountFromDouble(amount);
}

bool WalletManager::paymentIdValid(const QString &payment_id) const
{
    return Cred::Wallet::paymentIdValid(payment_id.toStdString());
}

bool WalletManager::addressValid(const QString &address, NetworkType::Type nettype) const
{
    return Cred::Wallet::addressValid(address.toStdString(), static_cast<Cred::NetworkType>(nettype));
}

bool WalletManager::keyValid(const QString &key, const QString &address, bool isViewKey,  NetworkType::Type nettype) const
{
    std::string error;
    if(!Cred::Wallet::keyValid(key.toStdString(), address.toStdString(), isViewKey, static_cast<Cred::NetworkType>(nettype), error)){
...
{
    return QString::fromStdString(Cred::Wallet::paymentIdFromAddress(address.toStdString(), static_cast<Cred::NetworkType>(nettype)));
}
...
{
    return Cred::Utils::isAddressLocal(daemon_address.toStdString());
}
...
{
    Cred::WalletManagerFactory::setLogLevel(logLevel);
}

void WalletManager::setLogCategories(const QString &categories)
{
    Cred::WalletManagerFactory::setLogCategories(categories.toStdString());
}
...
QString WalletManager::checkUpdates(const QString &software, const QString &subdir) const
{
  ...
  const std::tuple<bool, std::string, std::string, std::string, std::string> result = Cred::WalletManager::checkUpdates(software.toStdString(), subdir.toStdString());
  if (!std::get<0>(result))
    return QString("");
...
{
    m_pimpl =  Cred::WalletManagerFactory::getWalletManager();
}
