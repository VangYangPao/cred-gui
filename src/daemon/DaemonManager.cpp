...
{
    // prepare command line arguments and pass to credd
    ...
    // Start credd
    bool started = m_daemon->startDetached(m_credd, arguments);
    ...
    while(true && !m_app_exit) {
        ...
        if(running(nettype)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM credd.exe");
#else
                QProcess::execute("pkill credd");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./credd status` returns BUSY when syncing.
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to credd
#ifdef Q_OS_WIN
    m_credd = QApplication::applicationDirPath() + "/credd.exe";
#elif defined(Q_OS_UNIX)
    m_credd = QApplication::applicationDirPath() + "/credd";
#endif
