; Cred Lithium Luna GUI Wallet Installer for Windows
; Copyright (c) 2014-2018, The Cred Project
; See LICENSE

[Setup]
AppName=Cred GUI Wallet
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to Cred.

Source: "ReadMe.htm"; DestDir: "{app}"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy

; Cred GUI wallet
Source: "bin\cred-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion

; Cred GUI wallet log file
...
Source: "cred-wallet-gui.log"; DestDir: "{app}"; Flags: onlyifdoesntexist; Permissions: users-modify
...
; Cred daemon
Source: "bin\credd.exe"; DestDir: "{app}"; Flags: ignoreversion

; Cred daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "cred-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;
...
; Source: "bin\cred-utils-deserialize.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\cred-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  Result := BlockChainDir('') + '\bitcred.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "cred-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Cred daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\credd.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\cred-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "Cred GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\cred-wallet-gui.exe"
...
Name: "{group}\Utilities\Cred Daemon"; Filename: "{app}\credd.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\cred-wallet-cli.exe"; WorkingDir: "{userdocs}\Cred\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: "{userdocs}\Cred\wallets"
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: "{app}\cred-wallet-gui.log"
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\cred-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM credd.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{userdesktop}\GUI Wallet"; Filename: "{app}\cred-wallet-gui.exe"; Tasks: desktopicon
