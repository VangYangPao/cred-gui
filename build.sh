...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/cred-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    CREDD_EXEC=credd.exe
fi
...
get_tag
popd
echo "var GUI_CRED_VERSION = \"$TAGNAME\"" >> version.js
...
$QMAKE ../cred-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy credd to bin folder
if [ "$platform" != "mingw32" ] && [ "$ANDROID" != true ]; then
cp ../$CRED_DIR/bin/$CREDD_EXEC $BIN_PATH
fi
